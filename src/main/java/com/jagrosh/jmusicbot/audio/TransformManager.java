 ...
 */
public class TransformManager extends YoutubeAudioSourceManager
{
	private final static Logger log = LoggerFactory.getLogger(TransformManager.class);
	private final String name, regex, replacement, selector, format;

	public TransformManager(String name, Config object)
	{
		this(name, object.getString("regex"), object.getString("replacement"), object.getString("selector"), object.getString("format"));
	}

	public TransformManager(String name, String regex, String replacement, String selector, String format)
	{
		this.name = name;
		this.regex = regex;
	...
	public static List<TransformManager> createTransforms(Config transforms)
	{
		try
		{
			return transforms.root().entrySet().stream()
					.map(e -> new TransformManager(e.getKey(), transforms.getConfig(e.getKey())))
